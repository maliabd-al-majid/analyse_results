...
def plot_solved_wrt_treewidth_all_solvers(df, subtitle='', show_title=False):
    ...
    for i, (solver, group_df) in enumerate(grouped_by_solvers.items()):
        ...
        if solver != "nesthdb (treewidth >= 20 and #model > 10M) with clingo" and total==0:
            total= tuple(map(operator.add, unsolved_within_groups, solved_within_groups))
        #  print(solver)

        if solver == "nesthdb (treewidth >= 20 and #model > 10M) with clingo" and total!=0:
            unsolved_within_groups= tuple(map(operator.sub, total, solved_within_groups))
        ...
        _ = ax.bar(ind + i * (width + extra_space), unsolved_within_groups, width, color=ct.UNSOLVED_COLOR_BAR,
                   hatch=ct.HATCHES_DICT[solver], edgecolor='black')
        _ = ax.bar(ind + i * (width + extra_space), solved_within_groups, width, color=ct.SOLVED_COLOR_BAR,
                   hatch=ct.HATCHES_DICT[solver], edgecolor='black')
    ...
    handles = [mpatches.Patch(facecolor='white', edgecolor='black', hatch=ct.HATCHES_DICT[solver], label=solver) for
               solver, _ in grouped_by_solvers.items()]
